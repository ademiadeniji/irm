...
class VideoRecorder:
    def __init__(self, root_dir, render_size=256, fps=20, camera_id=0, use_wandb=False, frame_lst=False):
...
class TrainVideoRecorder:
    ...
    def record(self, obs):
        ...
        if self.enabled:
            if self.frame_lst:
                frames = [cv2.resize(o[-3:].transpose(1, 2, 0),
                                   dsize=(self.render_size, self.render_size),
                                   interpolation=cv2.INTER_CUBIC) for o in obs]
