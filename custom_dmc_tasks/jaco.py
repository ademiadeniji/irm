...
def make(task_id, obs_type, seed, time_limit=None, random_reset=True):
  obs_settings = observations.VISION if obs_type == 'pixels' else observations.PERFECT_FEATURES
  task = _reach(task_id, obs_settings=obs_settings, use_site=False, random_reset=random_reset)
...
class MTReach(composer.Task):
  ...
  def __init__(
      self, task_id, arena, arm, hand, prop, obs_settings, workspace, control_timestep, random_reset):
  ...
  def initialize_episode(self, physics, random_state):
    if not self.random_reset:
      random_state = np.random.mtrand.RandomState(0) # no random reset
...
def _reach(task_id, obs_settings, use_site, random_reset):
  ...
  task = MTReach(task_id, arena=arena, arm=arm, hand=hand, prop=prop,
               ...
               workspace=workspace,
               control_timestep=constants.CONTROL_TIMESTEP, random_reset=random_reset)
  return task