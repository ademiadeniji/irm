from irm.irm_gd import IRM_GD

import torch
import numpy as np

class IRM_GDIter(IRM_GD):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def run_skill_selection_method(self):
        best_skills = self.irm_gd_iter()
        return [dict(skill=sk) for sk in best_skills]

    def irm_gd_iter(self):   
        best_skills = []
        ...
        min_sk = self.irm_gradient_descent(extr_reward_id=0).cpu().detach().numpy()
        ...
        for r in range(1, self.n_rewards):
            ...
            min_sk = self.irm_gradient_descent(pearson_obs_th, pearson_next_obs_th, r).cpu().detach().numpy()
            best_skills.append(min_sk)
