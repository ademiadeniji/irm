...
from irm.irm_cem import IRM_CEM

import torch
import numpy as np

class IRM_CEMIter(IRM_CEM):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def run_skill_selection_method(self):
        best_skills = self.irm_cem_iter()
        return [dict(skill=sk) for sk in best_skills]

    def irm_cem_iter(self):   
        best_skills = []
        ...
        min_sk = self.irm_cem(extr_reward_id=0).cpu().numpy()
        ...
        for r in range(1, self.n_rewards):
            ...
            min_sk = self.irm_cem(pearson_obs_th, pearson_next_obs_th, r).cpu().numpy()
            best_skills.append(min_sk)
